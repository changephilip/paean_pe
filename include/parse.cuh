...
uint64_t _offset(char *chr)
{
    ...
    if (is_digitstr(chr_t))
        return ((atoi(chr_t) - 1) * refLength);
    else if (*chr_t == 'M')
        return (22 * refLength);
    else if (*chr_t == 'X')
        return (23 * refLength);
    ...
    while (sam_read1(fp, header, b) >= 0) {
        ...
        if (!((b->core).flag & BAM_FUNMAP) && (b->core).qual) {
            ...
            if ((b->core).flag & BAM_FPAIRED) {
                if ((b->core).mpos == 0 || !((b->core).flag & BAM_FREAD1))
                    continue;

                // get read2
                while (sam_read1(fp, header, b_mate) >= 0) {
                    if (((b_mate->core).flag & BAM_FREAD2) &&
                        (b_mate->core).pos == (b->core).mpos)
                        break;
                }
                ...
                //                if (!((b_mate->core).flag & BAM_FREAD2))
                //                    std::cout << b->core.flag <<
                //                    b_mate->core.flag << std::endl;
...
#define bam_startpos(b) \
    ((b->core).pos - bam_cigar2rlen((b->core).n_cigar, bam_get_cigar(b)))
                ...
                if (!bam_is_rev(b) && bam_is_rev(b_mate)) {
                    start_pos =
                        std::min((b_mate->core).mpos, bam_startpos(b_mate));
                    ...
                    for (int i = 1; i < (b_mate->core).n_cigar; i++) {
                        if ((cigars[i] & 15) == 3) {
                            read_core.junctions[read_core.junctionCount].end_ =
                                (b_mate->core).pos - start_pos - prev;
                            read_core.junctions[read_core.junctionCount]
                                .start_ = (b_mate->core).pos - start_pos -
                                          (prev + (cigars[i] >> 4));
                            read_core.junctionCount++;
                        }
                        prev = prev + (cigars[i] >> 4);
                    ...
                    for (int i = 1; i < (b->core).n_cigar; i++) {
                        if ((cigars[i] & 15) == 3) {
                            read_core.junctions[read_core.junctionCount].end_ =
                                (b->core).pos - start_pos - prev;
                            read_core.junctions[read_core.junctionCount]
                                .start_ = (b->core).pos - start_pos -
                                          (prev + (cigars[i] >> 4));
                            read_core.junctionCount++;
                        }
                        prev = prev + (cigars[i] >> 4);
...
//#define DEBUG
    ...
    for (int itr = 0; itr < h_reads.start_.size(); itr++) {
        std::cout << "read start coordinate: " << h_reads.start_[itr]
                  << std::endl;
        std::cout << "read end coordinate: " << h_reads.end_[itr] << std::endl;
        std::cout << "read strand: " << h_reads.strand[itr] << std::endl;
        std::cout << "read junction count: " << h_reads.core[itr].junctionCount
                  << std::endl;
...
void LoadReadFromBam(h_Reads &h_reads, h_nj_Reads &h_nj_reads, char *bam_file)
{
    samFile *fp;
    if ((fp = sam_open(bam_file, "r")) == NULL) {
        std::cerr << "Could not open this file: " << bam_file << std::endl;
        ...
        if (!((b->core).flag & BAM_FUNMAP) && (b->core).qual) {
            ...
            if (!((b->core).flag & BAM_FPAIRED)) {
                ...
                if (readJunctionCount !=0){
                        ...
                        h_nj_reads.start_.push_back((b->core).pos + offset + 1);
                        h_nj_reads.end_.push_back(bam_endpos(b) + offset + 1);
                        h_nj_reads.strand.push_back((uint32_t)(!bam_is_rev(b)));
                }
            } else {
                // if paried, PE
                if (qname_reg.empty()) {
                    ...
                    if (JunctionList.size() == 0) {
                        ...
                        //njread_strand = (uint32_t)(!bam_is_rev(b));
                    }
                    ...
                    if (qname_string.compare(qname_reg) == 0) {
                        ...
                        if (chrList.size() == 1) {
                            if (JunctionList.size() != 0) {
                                ...
                                left_startpos =
                                    *(std::min_element(start_posList.begin(),
                                                       start_posList.end()));
                                ...
                                if (junctionSet.size() > junctionSize) {
                                    printf(
                                        "catch a larger junctionCount in %s!\n",
                                        qname_reg.c_str());
                                }
                                ...
                                h_reads.strand.push_back(
                                    (uint32_t)(!bam_is_rev(b)));

                                h_reads.core.push_back(read_core);
                                // junctionSet.clear();
                            } else {
                                h_nj_reads.start_.push_back(njread_start);
                                h_nj_reads.end_.push_back(njread_end);
                                h_nj_reads.strand.push_back(
                                    (uint32_t)(!bam_is_rev(b)));
                            }
                        }
                        chrList.clear();
    ...
    if (JunctionList.size() != 0) {
        ...
        h_nj_reads.start_.push_back(njread_start);
        h_nj_reads.end_.push_back(njread_end);
        h_nj_reads.strand.push_back((uint32_t)(true));
    }
    bam_destroy1(b);
    /*
